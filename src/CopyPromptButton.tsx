  ...
  return (
    <div className="relative">
      <button type="button" onClick={() => void copy()}
        title={status === "copied" ? "已複製" : "複製咒語"}
        aria-label={status === "copied" ? "已複製咒語" : "複製咒語"}
      ...
      <span role="status" className="sr-only">{status === "copied" ? "已複製咒語" : ""}</span>
      {status === "error" && (
        <div role="alert" className="absolute right-0 top-full z-20 w-64 rounded-lg border border-[#2e2e32] bg-[#161618] p-3 text-xs text-[#c8c4bc]">
          ...
          <textarea readOnly aria-label="手動複製咒語" value={text} onFocus={(event) => event.currentTarget.select()} className="w-full rounded bg-[#0d0d0e] p-2" />
        </div>
      )}
    </div>
  );
